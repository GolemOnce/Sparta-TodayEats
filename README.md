...
### 🛒 주문 · 결제
- 가격은 서버에서 직접 계산, 주문 시점의 가게명·메뉴명·단가를 스냅샷으로 저장해 데이터 무결성 보장
- 상태 변경 시 DB 레벨 원자적 처리, 결제 시 비관적 락으로 동시성 제어
- 주문 취소·거절 시 자동 환불 처리 및 중복 결제 방지
- PENDING 상태에서 5분 이내만 취소 가능

### 🏪 가게 · 카테고리
- 역할별 조회 범위 세분화 (비로그인·Customer → 공개만 / Owner → 본인 가게 포함 / Manager·Master → 전체)
- QueryDSL로 카테고리·가게명 복합 조건 동적 검색 구현
- 이름 정규화 + 대소문자 무관 중복 체크, 소프트 삭제 후 재등록 가능
- 카테고리·운영지역 삭제 시 연관 가게 존재하면 삭제 불가

### 🔐 인증 · 사용자
- 이메일 인증(Redis) 완료 후 가입 가능
- Refresh Token 기반 Stateless 인증 구조 유지
- MASTER 권한 변경 차단, MANAGER는 DB 기반 동적 권한 검증
- 커스텀 어노테이션으로 사용자 ID 추출 및 권한 검증 로직 재사용

### 🤖 메뉴 · AI
- 역할별 메뉴 노출 정책 (Customer → 공개 메뉴만 / Owner → 숨김 포함 관리 가능)
- 품절·숨김 여부 별도 관리, 숨김·품절 메뉴 주문 차단
- Gemini AI로 메뉴 설명 자동 생성, 모든 AI 요청·응답 별도 테이블에 기록

### 💳 결제 · 리뷰 · 배송지
- 기존 도메인의 프로토타입을 기반으로 필요한 부분만 적용해 병합 시 충돌 최소화
- 글로벌 예외 처리·권한 검증 등 공통 모듈을 적극 활용해 유지보수성 확보
- 추후 기능 확장을 고려한 분기 처리 로직 설계 (결제 수단 확장 등)
- 완료된 주문에 한해 리뷰 작성 가능, 1~5점 평점 및 가게별 평균 평점 제공
